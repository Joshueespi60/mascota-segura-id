...
async function resolveFunctionErrorMessage(error) {
  ...
  if (errorContext && typeof errorContext.json === 'function') {
    try {
      ...
      // Ignoramos errores de parseo para usar el fallback del mensaje.
    }
  }
...
export async function unlinkPetFromQr(qrId) {
  ...
  const adminPin = (import.meta.env.VITE_ADMIN_QR_PIN || '').trim()
  if (!adminPin) {
    throw new Error('No se encontro VITE_ADMIN_QR_PIN para autorizar la desvinculacion.')
  }

  const { data, error } = await supabase.functions.invoke('unlink-pet-from-qr', {
    body: {
      qrId: normalizedQrId,
      adminPin,
    },
  ...
  if (error) {
    const functionMessage = await resolveFunctionErrorMessage(error)
    throw new Error(functionMessage || 'No se pudo desvincular la mascota de este QR.')
  }
  ...
  const mappedQr = mapQrCodeRow(data.qr)
  if (!mappedQr) {
    throw new Error('La funcion no devolvio el QR actualizado.')
  }
